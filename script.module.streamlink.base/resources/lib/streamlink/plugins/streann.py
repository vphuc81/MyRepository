...
class Streann(Plugin):
    ...
    stream_url = base_url + (
        "/loadbalancer/services/web-players/{type}s-reseller-secure/{dataId}/{playerId}/{token}/{resellerId}/playlist.m3u8?"
        "date={time}&device-type=web&device-name=web&device-os=web&device-id={deviceId}"
    )
