...
def searchGG(key, cx, string):
	#key = 'AIzaSyCszeAJOH1e3zrTtHmGFypth8M1adU1sEI'
	#cx = '009789051051551375973:rgvcpqg3uqw'
	href = 'https://www.googleapis.com/customsearch/v1?key=%s&cx=%s&q=%s'
	#log(href % (key, cx, string))
	b = xread(href % (key, cx, string))
	...
	for item in j.get('items', []):
		...
		if metatags:
			url = metatags.get('og:url', '')
			
			if not title:
				title = metatags.get('og:title', '')
			
			img = metatags.get('og:image', '')
		...
		if not img:
			try:
				img = item['cse_image'][0]['src']
			except:
				img = ""
	
			if not img:
				try:
					img = item['moviereview'][0]['image_href']
				except:
					img = ""
		
		if url and title:
			items.append((u2s(title), u2s(url), u2s(img)))
			
	
	try:
		startIndex = j['queries']['nextPage'][0]['startIndex']
	except:
		startIndex = ""
	
	if startIndex:
		title='[COLOR lime]Page next ...[/COLOR]'
		items.append((title,str(startIndex),""))
...
class serversList:
	def __init__(self):
		...
		'''
		(('hayhaytv.vn','23','tomato'), 'biphim.com','58','FFBA55D3'), ('phimsot.com','29','orangered')
	...
	def search(self, startIndex, string):
		key = 'AIzaSyCszeAJOH1e3zrTtHmGFypth8M1adU1sEI'
		cx = '009789051051551375973:rgvcpqg3uqw'
		href = 'https://www.googleapis.com/customsearch/v1?startIndex=%s&key=%s&cx=%s&q=%s'
		#href = 'https://www.googleapis.com/customsearch/v1element?start=%s&key=%s&rsz=filtered_cse&num=20&hl=vi&cx=%s&q=%s&cse_tok=ABPF6HiugIFMmWGlRClukRwD6XTiNVW3qg:1528296358942'
		b = xread(href % (startIndex, key, cx, string))
		...
		for item in j.get('items', []):
			...
			if url and title:
				items.append((u2s(title), url, img))
...
class googlesearch:
	...
	def content(self, cx, start, string, srv=''):
		self.url = 'https://www.googleapis.com/customsearch/v1element?key={key}&cx=009789051051551375973:{cx}&rsz=filtered_cse&num=20&hl=vi&prettyPrint=false&source=gcsc&gss=.vn&sig={sig}&cse_tok={cse_tok}&sort=&googlehost=www.google.com&callback=google.search.Search.apiary{apiary}&start={start}&q={string}'
		
		key = 'AIzaSyCVAXiUzRYsML1Pv6RwSG1gunmMikTzQqY'
		sig = 'f9d319213db9a87438e3102cff9a2ec9'
		
		if srv == 'free':
			cx = 'rgvcpqg3uqw'
			cse_tok = cse_tok = 'ABPF6HiWVOlPQFntoQCGcmOw2cELhIHkMQ:1525783598959'
			apiary = '18391'
		
		elif srv == 'fshare':
			cx = 'xekpkwarjri'
			cse_tok = 'ABPF6HivEgeKnkCt69qrGXKoyb8shyygSA:1525787872211'
			apiary = '13645'
			...
			cx = 'hzgiaiqusl0'
			cse_tok = 'ABPF6HgMmSRj0ZKOG9uHks-g3ouLZW5sgw:1525789602977'
			apiary = '3677'
		...
		href = self.url.format(
			...
			apiary = apiary,
			start = start,
			string = '%22' + '+'.join(string.split()) + '%22'
		)
		...
		if not d.get('results',{}):
			...
			try:
				d = json.loads(xsearch('\((\{.+?\})\)',xread(href.replace("%22", ""))))
			except:
				d = {}
		...
		start = ''.join(
			i.get('start','') for i in pages if i.get('label',0)==currentPage+2
			).encode('utf-8')
...
class fshare:#https://www.fshare.vn/home/Mục chia sẻ của thaitni/abc?pageIndex=1
	def __init__(self, username='', password='', id=''):
		self.url_id = id
		self.hd = {'User-Agent':'Mozilla/5.0','x-requested-with':'XMLHttpRequest', 'Cookie':''}
		self.logged = self.token = ''
		if username and password:
			self.user,self.passwd = username,password
		else:
			self.user,self.passwd = addon.getSetting('usernamef'),addon.getSetting('passwordf')
	...
	def login(self, user, passwd):
		if not user or not passwd:
			mess('Bạn set thông tin acc Fshare chưa đủ', 'Fshare.vn')
			return None
		
		try:
			b = urllib2.urlopen(urllib2.Request('https://www.fshare.vn/site/contact'))
			cookie = b.headers.getheader('set-cookie')
			fs_csrf = xsearch('"csrf-token" content="(.+?)"',b.read())
			#fs_csrf = xsearch('"_csrf-app" value="(.+?)"',b.read())
		except:
			cookie = fs_csrf = ''

		if fs_csrf:
			data = {"LoginForm[email]":user,"LoginForm[password]":passwd,"_csrf-app":fs_csrf}
			hd = {'Cookie':cookie}
			cookie = urllib2.HTTPCookieProcessor()
			opener = urllib2.build_opener(NoRedirection, cookie)
			urllib2.install_opener(opener)
			opener.addheaders=hd.items()
			o = opener.open('https://www.fshare.vn/site/login', urllib.urlencode(data))
			
			if o.getcode() == 302:
				cookie = xsearch('(fshare-app=.+?)\W', o.headers.get('Set-Cookie'))
				self.hd['Cookie'] = cookie
				#self.hd['Authorization'] = 'Bearer ' + xsearch('=(.+)', cookie)
				mess('Login thành công','Fshare.vn')
				self.logged = 'success'
			else:
				mess(u'Login không thành công!','Fshare.vn')
		else:
			mess(u'Login kthất bại!','Fshare.vn')
		
		return self.logged
		
	def login1(self,user,passwd):
		if not user or not passwd:
	...
	def get_folder(self,url):
		def fixTitleEPS(i):
			...
			try:
				if p[0]:
					i = re.sub(p[0],'Tập '+format(int(xsearch('(\d+)',p[0])),'02d'),i)
				elif p[1]:
					i = re.sub(p[1],'.E'+format(int(xsearch('(\d+)',p[1])),'02d'),i)
				elif p[2]:
					i = re.sub(p[2],format(int(xsearch('(\d+)',p[2])),'02d'),i)
			except:
				pass
			return i
	...
	def searchFollow(self, q, p=1):
		#b = xread('https://www.fshare.vn/files/searchFollow?key=%s'%q,self.hd)
		if q == 'hot':
			href = 'https://www.fshare.vn/file/follow'
		else:
			href = 'https://www.fshare.vn/file/follow-search?keyword=%s&page=%s'%(q, p)
		
		b = xread(href, self.hd)
		next = xsearch('<li class="next"><[^>]+?href="(.+?)"', b)
		
		b = [i for i in re.findall('(<tbody.+?/tbody>)', b, re.S) if 'onclick' in i]
		b = b[0] if b else ""
		items = re.findall('onclick="window.open\(\' (.+?)\'\)">(.+?)</a>', b)
		items = [(i[1], 'https://www.fshare.vn'+i[0]) for i in items]
		
		if next:
			items.append(('[COLOR lime]Trang tiếp theo ... %s[/COLOR]', next))
		'''
		else:
			b = xread('https://www.fshare.vn/file/follow-search?keyword=%s'%q,self.hd)
...
class fptPlay:#from resources.lib.servers import fptPlay;fpt=fptPlay(c)
	def __init__(self):
		...
		self.hd['referer'] = 'https://fptplay.vn/'
		...
		self.hd = {'Cookie':'laravel_id=1d71b90671c22a022178d16b69b342ce0e75dc15; token=%3D; laravel_session=','User-Agent':'Mozilla/5.0','Referer':'https://fptplay.vn/livetv','X-Requested-With':'XMLHttpRequest'}

	def login(self):
		phone   = get_setting('phone_fptplay')
		passwd  = get_setting('pass_fptplay')
		country = re.sub('\(.+?\)', '', get_setting('country_fptplay')) . strip()
		#log([phone, passwd, country])
		data    = urllib.urlencode({'phone':phone,'password':passwd,'country_code':country})
		
		#log(data)
		b       = urlfetch.post('https://fptplay.vn/user/login', data=data)
		if b.status == 302:
	...
	def liveLink(self,url):
		...
		if not id : 
			id = 'vtv3-hd'
		#id=su-kien-2&type=newchannel&quality=3&mobile=web
		data = 'mobile=web&quality=3&type=newchannel&id=%s'%id
		b = xread('https://fptplay.vn/show/getlinklivetv',self.hd,data)
		#log("b = xread('https://fptplay.vn/show/getlinklivetv',%s,'%s')" % (self.hd,data))
		#log(b)
		
		#try:
		link = json.loads(b).get('stream')
		if not link or "ttlogin" in link:
			#cliptv_cookie = xrw(os.path.join(home,'resources','lib','cliptv.cookie'))
			b = xread('https://cliptv.vn/truyen-hinh/'+id, {'Cookie':cliptv_cookie()})
			link = xsearch('"src\W+(.+?)"', b)
		
		else:
			link += '|User-Agent=Mozilla/5.0 AppleWebKit/537.36'
	...
	def playLink(self,url):
		...
		href = 'https://fptplay.vn/show/getlink'
		#href = 'https://fptplay.vn/show/getlinklivetv'
		if '?' not in url:
...
class phimoinet:
	...
	def getLink(self,url):
		...
		#links = j.get('medias',[]) + j.get('mediasBk5',[])
		links = j.get('medias',[])
		...
		#log(['a',links])
		if links:
			items = ls([(i.get('url'),rsl(i.get('resolution'))) for i in links if type(i) == dict])
			#log(items)
			#[(u'https://video.fhan3-3.fna.fbcdn.net/v/t42.9040-2/10000000_227083678011841_1575236353103233024_n.mp4?_nc_cat=0&efg=eyJybHIiOjE1MDAsInJsYSI6NDA5NiwidmVuY29kZV90YWciOiJzdmVfaGQifQ%3D%3D&rl=1500&vabr=781&oh=2e1b412685df51d9633d27d27afc3206&oe=5B644C50', '720')]
			...
			if not link:
				...
				def getc(href):
					...
					try:
						o = opener.open(href)
						cookie = o.headers.get('Set-Cookie')
						cookie = urllib.urlencode({'Cookie':cookie})
						link = o.info().dict["location"] + "|" + cookie
					except:
						link = ""
...
class kPhim:
	...
	def getLink(self,url):
		...
		#log(s)
		sl = [i for i in re.findall('(<div class="server-name".+?/div>)', b, re.S) if "sv=" in i]
		srvs = []
		for s in [i for i in sl if "sv=" in i]:
			sn = re.sub('<.+?>', '', xsearch('(<span.+?/span>)', s)).strip()
			for vid, sid, sv in re.findall('vid="(.+?)" sid="(.+?)".+?>(.+?)<', s):
				srvs.append((vid, sid, sn+' '+sv.strip()))
		if len(srvs) == 1:
			server_id,video_id,server = srvs[0]
		
		elif len(srvs)>1:
			label   = 'Chọn Server play phim này trên kphim.tv'
			choices = [i[2] for i in srvs]
			choice  = xselect(label,choices)
			
			if choice < 0:
				choice=0
			
			server_id, video_id, a = srvs[choice]
		else:
			return ''
		...
		log([b, url, server_id, video_id])
		href = xsearch('src="(.+?)"', b)
		
		
		if not href:
			items = re.findall("file\W+'(.+?)'.+?label\W+'(.+?)'", b, re.S)
			if items:
				return ls([(i[0],rsl(i[1])) for i in items])[0][0]
		
		if not href:
			link = xsearch("file\W+'(.+?)'", b)
			if link != 'http://dcm_chung_may.com':
				return link
		...
		elif href and 'openload.co' in href:
...
class hdVietnamn:#from resources.lib.servers import hdvn;hdvn=hdvn()
	def __init__(self):
		...
		self.hd = {
			...
			'Referer':'http://www.hdvietnam.com/forums/'
		}
		...
		if filetime('hdvietnam.cookie')>1:
			...
			self.hd['Cookie'] = xrw('hdvietnam.cookie')
		
		cookie = xread(self.urlhome + 'threads/997745/page-500', self.hd)
	...
	def login(self):
		...
		href = 'http://www.hdvietnam.com/login/login'
		data = {
				'login' : addon.getSetting('usernameh'),
				'password' : addon.getSetting('passwordh'),
				'register' : '0'
			}
		
		if not data['login'] or not data['password']:
			mess('Hãy set acc HDVietnam của bạn cho xshare!')
		...
		try:
			o = opener.open(href, data)
			cookie = o.headers.get('Set-Cookie')
			self.hd['Cookie'] = cookie
			xrw('hdvietnam.cookie', cookie)
			mess('Login HDVietnam thành công')
			
		except:
			cookie = ""
			mess('Xãy ra lỗi khi Login HDVietnam !')
	...
	def threads(self,url,refresh=''):
		...
		def srv(link):
			return [i for i in srvs if i in link]
		
		def getTitle(title,href,s):
			if refresh and 'fshare.vn' in href or '4share.vn' in href:
				return title
			...
			if [i for i in srvs if i in title] or not title:
				...
				if not title:
					title = xsearch('<title>(.+?)</title>',content)
		...
		_xfToken = xsearch('href="logout/\?_xfToken=(.+?)"', content)
		posts = re.findall('(<li id="post-.+?/li>)', content, re.S)
		if posts:
			post = posts[0]
			dataAuthor = xsearch('data-author="(.+?)"', post)
			posts = [i for i in posts if dataAuthor == xsearch('data-author="(.+?)"', i)]
			
			likeHref = xsearch('href="(.+?)" class="LikeLink item control like"', post)
			
			if likeHref:
				for post in posts:
					likeHref = xsearch('href="(.+?)" class="LikeLink item control like"', post)
					b=xread(
						'http://www.hdvietnam.com/'+likeHref,
						self.hd,
						'_xfToken=%s&_xfResponseType=json'%_xfToken
						)
					#log("b=xread('http://www.hdvietnam.com/%s',%s,'_xfToken=%s&_xfResponseType=json')"%(likeHref,self.hd,_xfToken))
				content = xread(url,self.hd)
				posts = re.findall('(<li id="post-.+?/li>)', content, re.S)
				posts = [i for i in posts if dataAuthor == xsearch('data-author="(.+?)"', i)]
...
class taiphim:
	...
	def getLink(self, b):
		def srvs(i):
			srv = [
				'fshare.vn','4share.vn','tenlua.vn','subscene.com','taiphimhd.com','phudeviet.org'
				]
			return [j for j in srv if j in i]
		
		def i_l(i,l):
			return i[0] not in [j[0] for j in l]
		
		l = []
		loop = False
		
		for s in re.findall('(<li id="post.+?/li>)', b, re.S):
			if 'click nút Like để lấy link' in s:
				s = self.getThanks(s)
				loop = True
			...
			if loop:
				...
				img = xsearch('img src="(.+?)"',s)
				p = '<a href="(.+?)" target="_blank"[^<]+?>(.+>)'
				l += [
					(i[0],re.sub('<[^<]+>','',i[1]),img) 
					for i in re.findall(p,s) if srvs(i[0])
					]
				
				p = 'href="([^"]+?)"[^<]+?><[^<]+?>([^<]+?)<'
				l += [
					(i[0],i[1],'') 
					for i in re.findall(p,s) if srvs(i[0]) and i[0] not in [j[0] for j in l]
					]
				
				l += [
					(i,i,img) 
					for i in re.findall('(http[^<]+?)<',s) if srvs(i) and i not in [j[0] for j in l]
					]
		
		n=[]
		for href,title,img in l:
			if re.search('"|,|\'|\?',href) or href in [j[0] for j in n]:
				continue
			
			s = b[b.find(href)-1000:
				b.find(href)]
			
			if xsearch('"(http[^"]+\.jpg)"',s):
			...
			if 'fshare.vn' in href or '4share.vn' in href:
				...
				while not i and fs>10:
					i = xsearch('style="font-size:.{,3}%d.{,5}">([^<]+)<' % fs, s)
					fs -= 1
			...
			if href and title.strip():
				n.append((href,' '.join(title.split()),img))
	...
	def getPage(self,url):
		...
		if not i:
			i = self.getLink(self.getContent(url))
		
		for href,title,img in i:
			if 'http' not in img:
				img = urlhome + img
			
			if 'http://taiphimhd.com/' in href:
				title = namecolor(title, self.c)
...
class sieunhanh:
	...
	def eps(self,url):
		items = []
		content = xread(url)
		s = xsearch('(<div class="video-list-row container".+?/ul>)',content,1,re.S)
	...
	def maxLink(self,url):
		...
		href = href+'?ip='+ip
		#log('b = xread("%s",%s)' % (href, self.hd))
		b = xread(href, self.hd)
		link = googleLinks(xsearch('sources\W+\[([^\]]+?)\]', b))
		...
		if not link:
			...
			link = googleItems([(i.get("file", ''), i.get("label", 0)) for i in j])
			#links = [(i.get("file", ''), i.get("label", 0)) for i in j.get("sources", {})]
			#link = googleLinks(links)
...
class hdonline:
	...
	def getLink1(self,url,ep='1'):
		...
		if xsearch('"file":"(.+?)"',b).replace('\\',''):
			...
			j = re.findall('(\{"file":".+?\})', b)
			j = [ xsearch('"file":"(.+?)"',i).replace('\\','') for i in j if 'ep='+ep+'&' in i]
			if j:
...
class vtvgovn:
	...
	def liveList(self):
		def detail(s):
			return xsearch('title="(.+?)"',s),xsearch('href="(.+?)"',s),xsearch('src="(.+?)"',s)
		
		b = xget(self.urlhome)
		if b and b.getcode()==200:
			xrw('vtvgo.cookie',b.headers.get('Set-Cookie'))
			b = b.read()
	...
	def live(self,id):
		...
		return link+'|'+urllib.urlencode({'Referer':'http://vtvgo.vn','User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.87 Safari/537.36','Cookie':xrw('vtvgo.cookie')})
...
class k88com:
	...
	def getLink(self,url):
		def getlink(b):
			...
			elif re.search('\{"file":".+?"\}', b):
				href = xsearch('(\{"file":".+?"\})', b)
				try:
					link = json.loads(href).get('link', '')
					if not link:
						link = json.loads(href).get('file', '')
				except:
					link = ''
			
			return link
		
		response = xsearch('(<div class="play".+?class="clear">)', xread(url), 1, re.S)
		hrefs = []
		#log(response)
		...
		if not link:
			try:
				response = re.sub('<!--.+?-->', '', response, flags=re.S)
			except:
				response = ""
...
class fcinenet:
	def __init__(self):
		...
		if not self.cookie or filetime('fcine.cookie')>1:
	...
	def login(self):
		...
		auth = addon.getSetting('fcine_user')
		pwd = addon.getSetting('fcine_pass')
		hd = {'User-Agent':'Mozilla/5.0'}
		
		if not auth or not pwd:
			pwd = 'eHNoYXJlQHRoYW5odGhhaS5uZXQrdGhhaXRuaQ=='
			auth,pwd = urllib2.base64.b64decode(pwd).split('+')
		...
		try:
			...
			b = o.read()
			o.close()
			csrfKey = xsearch('name="csrfKey" value="(.+?)"',b)
			ck = ';'.join('%s=%s'%(i.name,i.value) for i in cookie.cookiejar)
		except:
			csrfKey = ck = ""
	...
	def getFshare(self,url):
		...
		items = re.findall('href="(.+?)"[^<]*?>.+?>(.+?)</',s)
		#log(items)
		items = [i for i in items if ('fshare.vn' in i[0] and i[1]) or 'phụ đề' in i[1]]
		items = [(i[0],' '.join(re.sub('<.+?>','',i[1]).split())) for i in items ]

		return items
	
	def getFshare1(self,url):
		url='http://fcine.net/?controller=view&do=toggleFields&id=%s'%xsearch('/view/(\d+)-',url)
