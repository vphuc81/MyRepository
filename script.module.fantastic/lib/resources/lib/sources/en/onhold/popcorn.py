...
class source:
        ...
        try:
            ...
            for post in posts:
                try:
                    ...
                    for item in u:
                        'SD', info = source_utils.get_release_quality(item[0][0], None)
                        try:
                        ...
                        sources.append({'source': 'popcorn', 'quality': 'SD', 'language': 'en', 'url': url,
    ...
    def movie_links(self, link):
        try:
            ...
            POST = client.request('http://xmovies8.es/select-movie-quality/', post=post)
