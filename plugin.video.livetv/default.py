    ...
    if len(names) == 1:
        ...
        for item in items:
            ...
            if ("redirecttomovieshd" in link) or ("toyeuphim" in link) or ("redirecttonetmovie" in link) or \
               ...
               ("tofirefox" in link) or ("tosilk" in link) or ("tobeebtv" in link) or ("tosport365" in link) or ("tosupremacysports" in link):
                link = re.compile('<link>(.+?)</link>').findall(item)[0]            
...
def add_Link(name,url,iconimage):
    ...
    if 'tosilk' in url and apk:
        ...
        ok=xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=u,listitem=liz)
        return ok
    if 'tobeebtv' in url and apk:
        u = xbmc . executebuiltin ( 'StartAndroidActivity ( com.beeb.vn )' )
        ok=xbmcplugin.addDirectoryItem(handle=int(sys.argv[1]),url=u,listitem=liz)
        return ok
