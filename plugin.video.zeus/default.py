...
ZeusStreamDate = "11/10/2015 00:00hrs GMT"
...
HTMLPattern = "<(.*?)>"
DirectoryMSG = "[B][COLOR gold]CLICK ME[/COLOR][/B]"
...
print 'HakaMac zeusrepo.com ZeusAddonStartup#' + ZeusVideoBuild + '-' + str(osstat) + '  ' + str(ZeusStreamVersion) + '  ' + ZeusStreamDate
CompileData = 'NO DATA'
try: 
...
if CompileData == 'NO DATA': 
    ...
    print 'Hakamac zeusrepo.com no Compile Data * check connection to zeusrepo.com on this unit *'
else:
    print 'Hakamac zeusrepo.com read compile list successfully'
...
def Categories():
    ...
    if links.find('ZEUSMENU') < 1:
        print 'Hakamac zeusrepo.com cannot connect to the server'
...
def AddInfoLink():
    AddDir(DirectoryMSG,'wXaVd4Qk', 6, "http://s6.postimg.org/p9i4ct19d/newsletter.png", isFolder=True)
...
def ResolveIT():
    ...
    print 'Label Hakamac >>' + InfoLabel + '<<>>' + InfoLabel[-4:] + '<<Len>>' + str(len(InfoLabel)) + '<<'
