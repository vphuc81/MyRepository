...
menu = [
	( 'content', baseUrl, 'Featured Movies'),
	( 'content', baseUrl+'/recent_movies', 'Recently Added'),
...
def settings():
	addon.add_directory({'mode': 'meta'},{'title': 'Metadata Settings'}, img='https://raw.githubusercontent.com/Eldorados/script.module.metahandler/master/icon.png', is_folder=False)
#	addon.add_directory({'mode': 'adset'},{'title': 'Addon Settings'}, img=addon.get_icon(), is_folder=False)
...
def content( url ):
	...
	r = re.findall(r'<h3><a\stitle=\"(.*?)\s\(\d+.*?\"\shref=\"(.*?)\".*?\<b\>Year\<\/b\>\:\s(\d+)\s\-\s\<b\>Quality\<\/b\>\:\s(.*?)$', html, re.I|re.DOTALL|re.M)
	totalitems = len(r)
	if r:
		for name, url, year, quality in r:
			...
			if not os.path.exists(path):
				contextmenu_items.append(['[COLOR lime][B]A[/B]dd [B]T[/B]o [B]L[/B]ibrary[/COLOR]','XBMC.RunPlugin(%s)' % addon.build_plugin_url({'mode': 'add2lib', 'url': baseUrl+url, 'name': name, 'infol': meta, 'img': meta['cover_url'], 'fanart':meta['backdrop_url'], 'infol': 'libmeta', 'year': year})])
			if os.path.exists(path):
				contextmenu_items.append(['[COLOR red][B]R[/B]emove [B]F[/B]rom [B]L[/B]ibrary[/COLOR]','XBMC.RunPlugin(%s)' % addon.build_plugin_url({'mode': 'remromlib', 'url': baseUrl+url, 'name': name, 'infol': meta, 'img': meta['cover_url'], 'fanart':meta['backdrop_url'], 'infol': 'libmeta', 'year': year})])
			addon.add_video_item({'mode': 'playstream', 'url': baseUrl+url, 'name': name, 'infol': meta}, infolabels=meta, contextmenu_items=contextmenu_items, img=meta['cover_url'], fanart=meta['backdrop_url'], resolved=False, total_items=totalitems)
...
def playStreamUrl( url, infol, name ):
	...
	headers = {'host': 'm.afdah.org', 'content-type':'application/x-www-form-urlencoded; charset=UTF-8', 'origin':'https://m.afdah.org', 'referer': url, 'user-agent':'Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_3_2 like Mac OS X; en-us) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8H7 Safari/6533.18.5','x-requested-with':'XMLHttpRequest'}
	...
	res_videourl = []
	if '1080p' in data:
		res_quality.append( '[COLOR blue][B][1080p][/COLOR][/B]' )
		res_videourl.append( data["1080p"][3] )
	if '720p' in data:
		res_quality.append( '[COLOR green][B][720p][/COLOR][/B]' )
		res_videourl.append( data["720p"][3] )
	if '360p' in data:
		res_quality.append( '[COLOR red][B][360p][/COLOR][/B]' )
		res_videourl.append( data["360p"][3] )
	...
	if len(data) >1:
		...
		if ret == -1:
			...
			videourl = res_videourl[ret]
	else:
		videourl = res_videourl[0]

	videourl = urllib.unquote_plus(videourl)
	listitem = xbmcgui.ListItem(path=str(videourl), iconImage='', thumbnailImage='')
